...
const accessCategories = [
    {
        title: 'Parking & Transport', rating: 4.2,
        items: [
    ...
    {
        title: 'Accessible Bathrooms', rating: 4.2,
        items: [
    ...
    {
        title: 'In-Venue Accessibility', rating: 4.2,
        items: [
...
function StarRating({ rating }) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        stars.push(<Star key={i} size={14} stroke="#4543C6" fill={isFilled ? '#4543C6' : 'none'} />)
    }
            ...
            <main className="venue-content">
                ...
                <div className="venue-top">
                    <div className="venue-top__image-wrap">
                        <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/4/4a/Paramount_Theatre_Seattle.jpg/640px-Paramount_Theatre_Seattle.jpg" alt="Venue" className="venue-top__image" />
                    </div>
                    <div className="venue-top__info">
                        ...
                        <div className="venue-top__rating-row">
                            ...
                            <span className="venue-top__review-count">(25 Reviews)</span>
                        </div>
                        <div className="venue-top__address-row">
                            <MapPin size={14} className="venue-top__pin" />
                            <span>Address</span>
                        </div>
                        <div className="venue-top__actions">
                            <button className="btn-outline">View Reviews</button>
                            <button className="btn-solid">Write A Review</button>
                        </div>
                    </div>
                </div>
                    ...
                    {accessCategories.map((cat) => (
                        <div className="access-category" key={cat.title}>
                            <h3 className="access-category__title">
                                {cat.title} <Star size={13} stroke="#4543C6" fill="#4543C6" /> {cat.rating}
                            </h3>
                            <ul className="access-items">
                                {cat.items.map((item) => (
                                    <li key={item.label} className={`access-item access-item--${item.available ? 'yes' : 'no'}`}>
                                        <span className="access-item__icon">{item.available ? '✓' : '✕'}</span>
                                        {item.label}
                                    </li>
                                ))}
                            </ul>
                    ...
                    {reviews.map((review) => (
                        <article className="review-card" key={review.id}>
                            ...
                            <div className="review-card__footer">
                                ...
                                <button className="review-card__read">Read Review →</button>
                            </div>
                        </article>
                    ))}
                </div>
